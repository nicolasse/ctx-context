...
2. List directories at workspace root, skip `context/`, `node_modules/`, `.git/`
...
6. If `context/config.json` exists, read it and preserve existing `context` value unless user provided a new one
7. Replace `repos` array entirely with fresh scan results (URL strings only)
8. Sort repos alphabetically by URL
9. Write `context/config.json` with structure: `{ "context": "...", "repos": ["url1", "url2"] }`
10. Print summary: context URL, repos found, skipped directories

### UC-002: Pull repos

1. Read and parse `context/config.json`
...
Single file: `context/config.json`
